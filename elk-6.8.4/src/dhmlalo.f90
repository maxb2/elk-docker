...
pure subroutine dhmlalo(ias,ngp,ngpq,apwalm,apwalmq,dapwalm,dapwalmq,ld,dh)
use modmain
use modphonon
implicit none
! arguments
      ...
      do m3=-l3,l3
        ...
        do io=1,apword(l3,is)
          ...
          if (abs(dble(z1))+abs(aimag(z1)).gt.1.d-14) then
            ...
            do i=1,ngpq
              dh(i,j)=dh(i,j)+conjg(z1*apwalmq(i,io,lm3))
            end do
            i=ngpq+idxlo(lm1,ilo,ias)
            do j=1,ngp
              dh(i,j)=dh(i,j)+z1*apwalm(j,io,lm3)
            end do
          end if
        end do
      end do
        ...
        do m3=-l3,l3
          ...
          do io=1,apword(l3,is)
            ...
            if (abs(dble(z1))+abs(aimag(z1)).gt.1.d-14) then
              j=ngp+idxlo(lm1,ilo,ias)
              do i=1,ngpq
                dh(i,j)=dh(i,j)+conjg(z1*dapwalmq(i,io,lm3))
              end do
              i=ngpq+idxlo(lm1,ilo,ias)
              do j=1,ngp
                dh(i,j)=dh(i,j)+z1*dapwalm(j,io,lm3)
              end do
            end if
          end do
        end do
