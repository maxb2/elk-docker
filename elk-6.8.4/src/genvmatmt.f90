...
! determine alpha [PRB 67,153106 (2003)]
      ...
      do ispn=1,nspinor
        do m1=-l,l
          ...
          do jspn=1,nspinor
            do m2=-l,l
              lm2=idxlm(l,m2)
              sm1=sm1+dble(dm(lm1,ispn,lm2,jspn)*dm(lm2,jspn,lm1,ispn))
            end do
          end do
        end do
      end do
      ...
      sm2=n*(1.d0-0.5d0*n/dble(nm))-0.5d0*mg2/dble(nm)
      if (abs(sm2).gt.1.d-14) then
        alphadu(ia,i)=sm1/sm2
      else
        alphadu(ia,i)=0.d0
      end if
    end if
    ...
    do ispn=1,nspinor
      do m1=-l,l
        ...
        do jspn=1,nspinor
          do m2=-l,l
            lm2=idxlm(l,m2)
            sm1=sm1+dble(dm(lm1,ispn,lm2,jspn)*vmatmt(lm2,jspn,lm1,ispn,ias))
          end do
        end do
      end do
    end do
! subtract contribution to the energy of DFT+U potential
    engyadu(ia,i)=engyadu(ia,i)-sm1
! end loop over atoms
  end do
